...
<!--      最终选座：{{chooseSeatObj}}-->
  ...
  <a-modal v-model:visible="imageCodeModalVisible" :title="null"
    ...
    <p style="text-align: center;font-weight: bold;font-size: 18px">使用验证码</p>
    <p>
      <a-input v-model:value="imageCode" placeholder="请输入12306验证码">
        <template #suffix>
          <img v-show="!!imageCodeSrc" :src="imageCodeSrc" alt="验证码" v-on:click="loadImageCode()"/>
        </template>
      </a-input>
    </p>
    <a-button block @click="handleOk">提交订单</a-button>
  </a-modal>
...
export default defineComponent({
  ...
  setup() {
    ...
    const loadImageCode = () => {
      ...
      imageCodeSrc.value=process.env.VUE_APP_SERVER+'/business/image-code/'+imageCodeToken.value;
    };
